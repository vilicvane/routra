...
test('push', async () => {
  ...
  autorun(() => {
    observedUser = router.home.$view({parallel: true}).$entries[0].user;
  });

  expect(observedUser).toBe('vilicvane');
  expect(router.home.$view({parallel: true}).$entries[0].user).toBe(
    'vilicvane',
  );
  ...
  expect(router.home.$view({parallel: true}).$entries[1].user).toBe('zhang3');

  runInAction(() => {
    router.home.hello.$view({parallel: true}).$entries[0].user = 'li4';
  });
  ...
  expect(router.home.hello.$view({parallel: true}).$entries[0].user).toBe(
    'li4',
  );

  runInAction(() => {
    router.home.hello.$view({parallel: true}).$entries[0].user = 'wang5';
  });

  expect(observedUser).toBe('wang5');
  expect(router.home.$view({parallel: true}).$entries[0].user).toBe('wang5');
});
...
test('switch', async () => {
  ...
  expect(router.home.$view({parallel: true}).$entries[0].user).toBe('zhang3');
  expect(router.home.hello.$view({parallel: true}).$entries[0].user).toBe(
    'zhang3',
  );
